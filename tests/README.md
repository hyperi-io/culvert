...
This directory contains the test framework for the DFE VPN deployment.
...
Test full VPN connectivity against real infrastructure.

- **Framework:** BATS + SSH + OpenVPN/WireGuard client
- **Purpose:** Validate real VPN connections, OIDC flow, revocation
- **Dependencies:** SSH access to test VM, OpenVPN client, WireGuard tools
- **Run time:** 5-15 minutes

```bash
# Configure test environment
cp tests/e2e/config.env.example tests/e2e/config.env
vim tests/e2e/config.env

# Run e2e tests (requires VPN infrastructure)
./tests/run-e2e.sh
```
