...
class TestMainValidatesBeforeRun:
    """main('server') must validate config before starting the server."""

    def test_server_command_validates_then_runs(self, monkeypatch):
        entrypoint = importlib.import_module("entrypoint")

        calls = []
        cfg = Config(server_cn="vpn.example.com", protocol="wireguard")
        monkeypatch.setattr(
            entrypoint.Config, "from_settings", classmethod(lambda cls: cfg)
        )
        monkeypatch.setattr(cfg, "validate", lambda: calls.append("validate"))
        monkeypatch.setattr(entrypoint, "run_server", lambda c: calls.append("run"))
