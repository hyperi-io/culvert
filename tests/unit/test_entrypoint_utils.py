...
"""
    ...
    def test_strips_dfe_vpn_version_comments(self):
        """Strips DFE VPN version comments."""
        content = """# DFE VPN 1.2.3
key=value
"""
        result = _strip_timestamps(content)
        assert "DFE VPN 1.2.3" not in result
        assert "key=value" in result
...
# with direct hyperi_pylib.logger usage. Those are tested upstream.
