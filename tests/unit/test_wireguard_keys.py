...
class TestAllocatePeerIp:
    ...
    def test_concurrent_allocations_do_not_collide(self, pki_dir: Path) -> None:
        """Parallel allocations get unique IPs and leave valid JSON (flock)."""
        import threading
        ...
        def worker(name: str) -> None:
            try:
                results.append(allocate_peer_ip(pki_dir, "10.8.0.0/24", name))
            except Exception as exc:
                errors.append(exc)

        threads = [
            threading.Thread(target=worker, args=(f"client{i}",)) for i in range(12)
        ]
        for t in threads:
            t.start()
        for t in threads:
        ...
        assert not errors
        assert len(set(results)) == len(results)  # every IP unique
        alloc = json.loads((wg_dir / "allocations.json").read_text())
        assert len(alloc) == 12

    def test_reallocating_same_name_is_idempotent(self, pki_dir: Path) -> None:
        """Allocating an existing name returns its current IP, not a new one."""
        wg_dir = pki_dir / "wireguard"
        wg_dir.mkdir(parents=True)

        first = allocate_peer_ip(pki_dir, "10.8.0.0/24", "alice")
        again = allocate_peer_ip(pki_dir, "10.8.0.0/24", "alice")
        assert first == again
