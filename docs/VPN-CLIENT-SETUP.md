...
sudo cp alice-wg-split.conf /etc/wireguard/hyperi-vpn.conf

# Enable and start
sudo systemctl enable wg-quick@hyperi-vpn
sudo systemctl start wg-quick@hyperi-vpn

# Check status
sudo systemctl status wg-quick@hyperi-vpn
```
...
# Import and connect
openvpn3 config-import --config alice-udp-split.ovpn --name hyperi-vpn
openvpn3 session-start --config hyperi-vpn
```
...
sudo cp alice-udp-split.ovpn /etc/openvpn/client/hyperi-vpn.conf

# Enable and start
sudo systemctl enable openvpn-client@hyperi-vpn
sudo systemctl start openvpn-client@hyperi-vpn

# Check status
sudo systemctl status openvpn-client@hyperi-vpn
```
...
Before=openvpn-client@hyperi-vpn-https.service

[Service]
Type=forking
ExecStart=/usr/bin/stunnel /etc/openvpn/client/hyperi-vpn-stunnel.conf
...
```bash
sudo mkdir -p /etc/openvpn/client
sudo cp alice-stunnel.conf /etc/openvpn/client/hyperi-vpn-stunnel.conf

# Edit to run as daemon (change foreground = yes to foreground = no)
sudo sed -i 's/foreground = yes/foreground = no/' /etc/openvpn/client/hyperi-vpn-stunnel.conf
```

Copy OpenVPN config:

```bash
sudo cp alice-https-split.ovpn /etc/openvpn/client/hyperi-vpn-https.conf
```

Create OpenVPN service dependency:

```bash
sudo mkdir -p /etc/systemd/system/openvpn-client@hyperi-vpn-https.service.d
sudo tee /etc/systemd/system/openvpn-client@hyperi-vpn-https.service.d/override.conf << 'EOF'
[Unit]
...
sudo systemctl enable openvpn-client@hyperi-vpn-https
sudo systemctl start stunnel-vpn.service
sudo systemctl start openvpn-client@hyperi-vpn-https

# Check status
sudo systemctl status stunnel-vpn.service
sudo systemctl status openvpn-client@hyperi-vpn-https
```
...
- **wstunnel connection refused** — ensure DPI bypass is enabled on the server (`WIREGUARD_DPI_BYPASS_ENABLED=true`)
...
your administrator via `HYPERI_VPN_PUSH_ROUTES`. Consult your site's
