...
from hyperi_pylib.logger import logger  # noqa: E402
...
class Config:
    ...
    Wraps lib.config.Config (HYPERI_VPN_* cascade) and adds output_dir +
...
def generate_ovpn_config(
    ...
    if protocol == "udp":
        ...
        stunnel_config = f"""# HyperI VPN stunnel Client Configuration
...
"""
    ...
    # Build config
    config = f"""# HyperI VPN Client Configuration
...
"""
    ...
    if stunnel_config:
        ...
        stunnel_path.write_text(stunnel_config)
        stunnel_path.chmod(0o600)
...
def generate_wireguard_configs(
    ...
    wg0_path.write_text(server_conf)
    wg0_path.chmod(0o600)
...
""",
    ...
    if not validate_client_name(client_name):
        logger.error(
            f"Invalid client name '{client_name}'. Use only alphanumeric, dash, underscore."
        )
        sys.exit(1)
    ...
    if generate_openvpn:
        ...
        if args.config_only:
            ...
            if cert_path.exists():
                ...
                logger.error(
                    "Use revoke-client to revoke first, or use --config-only to regenerate configs"
                )
                sys.exit(1)
