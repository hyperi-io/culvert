...
"""
OAuth2/OIDC SSO configuration for dfe-vpn.
...
def setup_oauth2(cfg) -> None:
    ...
    if not http_secret:
        logger.warning("OAUTH2_HTTP_SECRET not set, generating random secret")
    ...
    Path("/etc/vpn/management.pwd").write_text(mgmt_password)
    Path("/etc/vpn/management.pwd").chmod(0o600)
...
def start_oauth2(cfg) -> None:
    """Start openvpn-auth-oauth2 instances."""
    any_oauth2 = (
        cfg.oauth2_udp_enabled or cfg.oauth2_tcp_enabled or cfg.oauth2_https_enabled
    )
    ...
    if cfg.udp_enabled and Path("/etc/openvpn-auth-oauth2/config-udp.yaml").exists():
        run(
            "openvpn-auth-oauth2 --config /etc/openvpn-auth-oauth2/config-udp.yaml &",
            check=False,
        )
        logger.info(f"  oauth2-udp started (port {cfg.oauth2_udp_port})")
        started += 1

    if (
        cfg.https_enabled
        and Path("/etc/openvpn-auth-oauth2/config-https.yaml").exists()
    ):
        run(
            "openvpn-auth-oauth2 --config /etc/openvpn-auth-oauth2/config-https.yaml &",
            check=False,
        )
        logger.info(f"  oauth2-https started (port {cfg.oauth2_https_port})")
        started += 1

    if cfg.tcp_enabled and Path("/etc/openvpn-auth-oauth2/config-tcp.yaml").exists():
        run(
            "openvpn-auth-oauth2 --config /etc/openvpn-auth-oauth2/config-tcp.yaml &",
            check=False,
        )
        logger.info(f"  oauth2-tcp started (port {cfg.oauth2_tcp_port})")
        started += 1
